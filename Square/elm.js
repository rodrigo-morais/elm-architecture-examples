...
Elm.SpinSquarePair.make = function (_elm) {
   ...
   if (_elm.SpinSquarePair.values) return _elm.SpinSquarePair.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($SpinSquare.view,A2($Signal.forwardTo,address,Left),model.left),A2($SpinSquare.view,A2($Signal.forwardTo,address,Right),model.right)]));
   });
   var Model = F2(function (a,b) {    return {left: a,right: b};});
   var init = function () {
      var _p0 = $SpinSquare.init;
      var right = _p0._0;
      var rightFx = _p0._1;
   ...
   var update = F2(function (action,model) {
      var _p2 = action;
      if (_p2.ctor === "Left") {
            var _p3 = A2($SpinSquare.update,_p2._0,model.left);
            var left = _p3._0;
            var fx = _p3._1;
            return {ctor: "_Tuple2",_0: A2(Model,left,model.right),_1: A2($Effects.map,Left,fx)};
         } else {
            var _p4 = A2($SpinSquare.update,_p2._0,model.right);
            var right = _p4._0;
            var fx = _p4._1;
            return {ctor: "_Tuple2",_0: A2(Model,model.left,right),_1: A2($Effects.map,Right,fx)};
         }
   });
   return _elm.SpinSquarePair.values = {_op: _op,Model: Model,init: init,Left: Left,Right: Right,update: update,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $SpinSquarePair.init,update: $SpinSquarePair.update,view: $SpinSquarePair.view,inputs: _U.list([])});
   var main = app.html;
